...
source("C:/Users/33701/Desktop/code/network_ordinal.R")
source("C:/Users/33701/Desktop/code/Y_only_ordinal.R")
...
res = foreach(iter = 1:replation, .verbose = TRUE, .packages = c("MASS","pgdraw","LaplacesDemon","truncnorm"), .combine = list, .multicombine = TRUE) %dopar% {
  ...
  for (i in 1:length(cn)) {
    ...
    Y = get_item_Y(Z_ture,B_ture,b1,b2,b3,b4)
    data_ture[[i]] = list("alpha" = alpha_ture,"Z" = Z_ture,"b1" = b1,"b2" = b2,"b3" = b3,"b4" = b4,"B" = B_ture,"A" = A,"Y" = Y)
    net_out = network_only_1123(A, 1314, nrun,burn,thin, delta_n, alpha_H,a_theta, b_theta, 
                                theta_inf, start_adapt, Hmax, alpha0,alpha1)
    net_Y_out = joint_item_Y(A,Y,nrun,burn,thin,delta_n,alpha_H,a_theta,b_theta,
                             theta_inf,Hmax,start_adapt,alpha0,alpha1)
    Y_out = item_Y_only(Y,nrun,burn,thin,delta_n,alpha_H,a_theta_B,b_theta_B,
                        theta_inf = 0.05,Hmax,start_adapt,alpha0,alpha1)
    out_net[[i]] = net_out
