...
network_mix = function(A,Y1,Y2,Y3,nrun,burn,thin,delta_n,alpha_H,a_theta,b_theta,a_sig,b_sig,
  ...
  for (run in 1:nrun){
    ...
    for(i in 1:n){
      ...
      u_Zi = Sigma_Zi %*% ((t(Z_i) %*% (kappa_Ai - diag(D_Ai,nrow = (n -1)) %*% alp_cons)) 
                           + (B1 %*% diag(inv_sigma,nrow = q1) %*%(Y1[i,] - gamma1))
    ...
    for(j in 1:q1){
      sigma_B1j = chol2inv(chol(diag(H, nrow = H) + inv_sigma[j] * t(Z) %*% Z))
    ...
    for(j in 1:q2){
      D_Y2j = D_Y2[,j]
      sigma_B2j = chol2inv(chol(diag(H, nrow = H) + t(Z) %*% diag(D_Y2j, nrow = n) %*% Z))
    ...
    for (j in 1:q3) {
      W_j = W[,j]
      phi_j = Phi[,j]
      sigma_B3j = chol2inv(chol(t(Z)%*%diag(W_j,nrow = n)%*%Z + diag(H, nrow = H)))
  ...
  output = list("H" = H_hat,"alpha" = alpha_hat,"B1" = B1_hat,"B2" = B2_hat,"B3" = B3_hat,
